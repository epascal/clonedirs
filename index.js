    ...
    walkCopy(dir) {
        ...
        for (let file of list) {
            ...
            if (stat && stat.isDirectory()) {
                ...
                let bigFilesList = this.bigFilesMapDest.get(fileName);
    ...
    walkDelete(dir) {
        ...
        for (let file of list) {
            ...
            if (stat && stat.isDirectory()) {
                ...
                let srcFile = file.replace(scope.dstFolder, scope.srcFolder);
                let fileName = file.substr(file.lastIndexOf('/') + 1);
                let bigfileList = this.bigFilesMapSrc.get(fileName);
            ...
            if (stat && stat.isDirectory()) {
                ...
                if (stat.size > 10000000 && fileName.length > 10) {
                    if (this.bigFilesMapDest.get(fileName)) {
                        this.bigFilesMapDest.get(fileName).push({ 'file': file, 'size': stat.size });
                    }
                    else {
